...
(* domain = linguistic  built = 2026-07-18T13:04:07.537700+00:00 *)
...
FSOTTrace[] := Module[{steps = {}, c, add,
  ...
  S = k*raw; add["S", S];
  Association["domain" -> "linguistic", "S" -> S, "T1" -> t1, "T2" -> t2, "T3" -> t3, "steps" -> steps]
];

(* Python microscope reference panel for cross-check *)
FSOTPythonPanel = <|"S" -> 0.65132476188489685, "T1" -> 0.54995521990942409, "T2" -> 1, "T3" -> 1.7070633668184452e-17|>;
